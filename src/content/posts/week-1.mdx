...
dateEnd: 2026-06-28
pubDate: 2026-06-28
summary: "serf ships v0.1.0 on the back of a 414-commit week, toil lands as a new workflow orchestrator, and a 90-commit credential-axis push reshapes the eval lab. Eleven repos active across both orgs."
---

import WeekSummary from '../../components/WeekSummary.astro';

The first week we're recapping opens with a release we'd been building toward: **serf** shipped its `v0.1.0` — on top of a 414-commit week. And that's one repo of eleven that saw activity across `prime-radiant-inc` and `obra`, from a brand-new Go workflow orchestrator to a 90-commit reshaping of how the eval lab handles model credentials.

<section class="summary">

<WeekSummary week={1} />

<p class="caveat"><em>A note on the line count: this was a going-public week, so a large share of those added lines is toil's one-time initial public release (+111,838 across 519 files) rather than organic day-to-day change. We flag it rather than let the number oversell the week.</em></p>
...
Two commits from Jesse Vincent, both about authentication: support for Claude subscription OAuth on the Anthropic client, then a fix to pin `apiKey: null` in OAuth mode so the SDK sends only a Bearer token.
...
No direct commits to the default branch in our window, but six PRs merged — a Codex-focused cleanup run: removing Gemini CLI support now that Google has EOLed it, adding a Codex marketplace manifest, pruning per-harness tool-mapping boilerplate, removing Codex hooks, compressing the `using-superpowers` bootstrap, and stopping the `SessionStart` bootstrap from re-firing on resume.
